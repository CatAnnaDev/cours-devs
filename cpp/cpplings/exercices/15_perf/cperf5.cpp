...
long long somme(Poignee poignee) {
    long long total = 0;
...
int maximum(Poignee poignee) {
    int plus_grand = poignee.partagee->valeurs.front();
    for (const int valeur : poignee.partagee->valeurs) {
...
std::size_t combien(Poignee poignee) { return poignee.partagee->valeurs.size(); }
