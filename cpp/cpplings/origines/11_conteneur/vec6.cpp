...
long long somme_des_valeurs(const Vecteur<verif::Sonde> &vecteur) {
    long long somme = 0;
    for (std::size_t i = 0; i < vecteur.capacite(); i++) {
...
int main() {
    ...
    const verif::Sonde &premier = vecteur[0];
    vecteur.emplace_back(5);

    VERIFIE_ENTIER(vecteur.capacite(), 8, "l'ajout de trop a fait doubler le tampon");
    VERIFIE_ENTIER(premier.valeur, 1, "le premier element est encore la");
    VERIFIE(&premier == &vecteur[0], "et il n'a pas bouge");
