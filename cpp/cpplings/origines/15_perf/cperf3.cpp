...
std::size_t decouper_en_vues(std::string_view source, std::string_view sortie[],
                             std::size_t capacite) {
    static std::string tampon[NOMBRE_DE_CHAMPS];
    std::size_t compte = 0;
    std::size_t debut = 0;
    while (debut <= source.size() && compte < capacite && compte < NOMBRE_DE_CHAMPS) {
        const std::size_t fin = source.find(SEPARATEUR, debut);
        const std::size_t longueur = (fin == std::string_view::npos ? source.size() : fin) - debut;
        tampon[compte] = std::string(source.substr(debut, longueur));
        sortie[compte] = tampon[compte];
        compte++;
