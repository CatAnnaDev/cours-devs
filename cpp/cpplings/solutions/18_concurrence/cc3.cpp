...
constexpr long PLAFOND_D_ESSAIS = 10000000;
...
void empiler(Buche *buche) {
    ...
    for (long essai = 0; essai < PLAFOND_D_ESSAIS; essai++) {
        buche->dessous = ancien_sommet;
        laisser_passer_l_autre_fil();
        if (sommet.compare_exchange_weak(ancien_sommet, buche, std::memory_order_release,
                                         std::memory_order_relaxed)) {
            return;
        }
    }
    poussees_abandonnees.fetch_add(1, std::memory_order_relaxed);
}
