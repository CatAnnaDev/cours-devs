...
Generateur toutes_les_tables(int tables, int longueur) {
    for (int facteur = 1; facteur <= tables; facteur++) {
        for (int rang = 1; rang <= longueur; rang++) {
            co_yield facteur * rang;
        }
    }
}
...
long long somme_des_tables(int &valeurs_produites) {
    long long total = 0;
    for (int valeur : toutes_les_tables(NOMBRE_DE_TABLES, LONGUEUR_DE_TABLE)) {
        total += valeur;
        valeurs_produites++;
    }
    return total;
