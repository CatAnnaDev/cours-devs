...
public:
    ...
    void reserve(std::size_t capacite_voulue) {
        ...
        std::size_t construits = 0;
        try {
            for (; construits < taille_; construits++) {
                new (nouveau + construits) T(std::move_if_noexcept(stockage_[construits]));
            }
        } catch (...) {
            for (std::size_t i = construits; i > 0; i--) {
                nouveau[i - 1].~T();
            }
            ::operator delete(nouveau);
            throw;
        }
        detruire_les_elements();
        ::operator delete(stockage_);
