...
int masse_salariale(const std::vector<std::unique_ptr<Employe>> &equipe) {
    int total = 0;
    for (const std::unique_ptr<Employe> &membre : equipe) {
...
int main() {
    ...
    std::vector<std::unique_ptr<Employe>> equipe;
    equipe.reserve(2);
    equipe.push_back(std::make_unique<Employe>(1000));
    equipe.push_back(std::make_unique<Commercial>(1000, 50, 4));
