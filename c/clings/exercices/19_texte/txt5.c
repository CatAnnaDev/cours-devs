...
static void en_majuscules(char *texte) {
    for (size_t indice = 0; texte[indice] != '\0'; indice++) {
        unsigned char octet = (unsigned char)texte[indice];
        unsigned char haut = (unsigned char)toupper(octet);

        if (haut == octet && octet >= 0xE0 && octet <= 0xFE) {
            haut = (unsigned char)(octet - 32);
        }
        if (haut != octet) {
            texte[indice] = (char)haut;
            octets_modifies++;
        }
    }
}
