...
static uint64_t bits_de_reel(double valeur) {
    uint64_t bits;
    memcpy(&bits, &valeur, sizeof bits);
    return bits;
}

static double reel_de_bits(uint64_t bits) {
    double valeur;
    memcpy(&valeur, &bits, sizeof valeur);
    return valeur;
}

static void ecrire_reel_gros_boutiste(uint8_t *sortie, double valeur) {
    uint64_t bits = bits_de_reel(valeur);
    for (unsigned i = 0; i < REEL_OCTETS; i++) {
        sortie[i] = (uint8_t)(bits >> (56u - 8u * i));
    }
}
...
static double lire_reel_gros_boutiste(const uint8_t *octets) {
    uint64_t bits = 0;
    for (unsigned i = 0; i < REEL_OCTETS; i++) {
        bits = (bits << 8) | (uint64_t)octets[i];
    }
    return reel_de_bits(bits);
}
