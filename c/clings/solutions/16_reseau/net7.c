...
static int preparer_l_emetteur(int prise) {
    int un = 1;
    if (setsockopt(prise, IPPROTO_TCP, TCP_NODELAY, &un, sizeof un) != 0) {
        return -1;
    }
    return setsockopt(prise, SOL_SOCKET, SO_NOSIGPIPE, &un, sizeof un);
}
...
static int interpreter_l_ecriture(ssize_t retour, int erreur) {
    ...
    if (erreur == EPIPE || erreur == ECONNRESET) {
        return PAIR_PARTI;
    }
