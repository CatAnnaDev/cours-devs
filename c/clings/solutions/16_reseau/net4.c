...
static Envoi tout_envoyer(int prise, const char *source, size_t taille) {
    Envoi bilan = {0, 0, 0, 0};
    while (bilan.octets_ecrits < taille) {
        ssize_t ecrits = write(prise, source + bilan.octets_ecrits, taille - bilan.octets_ecrits);
        bilan.appels_a_write++;
        if (ecrits > 0) {
            bilan.octets_ecrits += (size_t)ecrits;
            continue;
        }
        if (ecrits < 0 && errno == EINTR) {
            continue;
        }
        if (ecrits < 0 && errno != EAGAIN) {
            bilan.erreur = errno;
            break;
        }
        bilan.attentes++;
        if (!attendre_de_la_place(prise, DELAI_MAX_MS)) {
            bilan.erreur = ETIMEDOUT;
            break;
        }
    }
    return bilan;
