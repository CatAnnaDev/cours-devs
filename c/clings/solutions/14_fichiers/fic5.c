...
static int remplacer_contenu(const char *chemin, const char *const *lignes, size_t nombre,
                             size_t echec_apres) {
    char temporaire[512];
    int ecrits = snprintf(temporaire, sizeof temporaire, "%s.neuf", chemin);
    if (ecrits < 0 || (size_t)ecrits >= sizeof temporaire) {
        return -1;
    }
    ...
    if (ecrire_lignes(flux, lignes, nombre, echec_apres) != 0 || fflush(flux) != 0) {
        fclose(flux);
        remove(temporaire);
        return -1;
    }
    if (fclose(flux) != 0 || rename(temporaire, chemin) != 0) {
        remove(temporaire);
        return -1;
    }
    return 0;
