...
static size_t decoder_une_sequence(const unsigned char *octets, uint32_t *point) {
    ...
    if (tete < 0xE0) {
        *point = ((uint32_t)tete << 6) | octets[1];
        return 2;
    }
    if (tete < 0xF0) {
        *point = ((uint32_t)tete << 12) | ((uint32_t)octets[1] << 6) | octets[2];
        return 3;
    }
    *point = ((uint32_t)tete << 18) | ((uint32_t)octets[1] << 12) |
             ((uint32_t)octets[2] << 6) | octets[3];
    return 4;
}
