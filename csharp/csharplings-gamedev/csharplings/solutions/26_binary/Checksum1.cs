...
public static class Checksum1
{
    ...
    public static bool TryOpen(ReadOnlySpan<byte> sealedContent, out byte[] payload)
    {
        ...
        if (Hash(body) != BitConverter.ToUInt32(sealedContent.Slice(sealedContent.Length - 4)))
            return false;
    ...
    public static void SaveAtomically(FakeDisk disk, string path, ReadOnlySpan<byte> payload)
    {
        ...
        disk.Write(temporary, Seal(payload));

        if (!TryOpen(disk.Read(temporary), out _))
        {
            disk.Delete(temporary);

            throw new IOException("relecture impossible, sauvegarde abandonnee");
        }
