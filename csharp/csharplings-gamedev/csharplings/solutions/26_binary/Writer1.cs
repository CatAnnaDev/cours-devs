...
public ref struct BlobWriter
{
    ...
    {
        if (Written + count > _destination.Length)
            throw new InvalidOperationException($"tampon plein : il faut {count} octets, il en reste {_destination.Length - Written}");
...
public ref struct BlobReader
{
    ...
    {
        if (Read + count > _source.Length)
            throw new InvalidOperationException("fichier tronque");
