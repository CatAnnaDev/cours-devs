...
public static class Sort1
{
    ...
    public static List<string> ByScore() =>
        Board.OrderByDescending(runner => runner.Score).Select(runner => runner.Name).ToList();
    ...
    public static Runner Best() => Board.MaxBy(runner => runner.Score);

    public static Runner Worst() => Board.MinBy(runner => runner.Score);
