...
public static class Group1
{
    ...
    public static List<string> ItemsOfZones(IEnumerable<string> zones) => Todo.Value<List<string>>();
    ...
    public static Dictionary<string, int> WeightPerZone() =>
        Zones.Join(Drops, zone => zone.Name, drop => drop.Zone, (zone, drop) => (zone.Name, drop.Weight))
            .GroupBy(pair => pair.Name)
            .ToDictionary(group => group.Key, group => group.Sum(pair => pair.Weight));
