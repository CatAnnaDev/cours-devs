...
public static class Create1
{
    ...
    public static bool TryWriteHealth(Span<char> destination, int current, int max, out int written)
    {
        ...
        current.TryFormat(destination.Slice(written), out int digits, provider: CultureInfo.InvariantCulture);
    ...
    public static string Repeat(char character, int count) => Todo.Value<string>();
